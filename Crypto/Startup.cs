...
namespace Crypto
{
    ...
    { 
        ...
        public void ConfigureServices(IServiceCollection services)
        {
            ...
            {
                services.AddDbContext<CryptoDbContext>(options => options.UseSqlServer(mssqlserverConnectionString));
            }
            else if (!string.IsNullOrEmpty(sqliteConnectionString))
            {
                services.AddDbContext<CryptoDbContext>(options => options.UseSqlite(sqliteConnectionString));
            }
        ...
        public void Configure(IApplicationBuilder app, IHostingEnvironment env)
        {
            using (var serviceScope = app.ApplicationServices
                ...
                .CreateScope())
            {
                using (var context = serviceScope.ServiceProvider.GetRequiredService<CryptoDbContext>())
                {
